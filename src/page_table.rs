...
impl<'a> MutPageTableWrapper<'a> {
    ...
    pub fn map_kernel(&mut self, vaddr: usize, paddr: usize) -> Result<usize, String> {
        ...
        self.page_table.map_frame_for_kernel(vaddr & mask, paddr - KERNEL_OFFSET, 0);
        Ok(aligned_size)
    }
        ...
        match self.search_for_insert(vaddr).unwrap() {
            (SearchResult::Missing(level), page_table) => {
                if level == HAL_PAGE_LEVEL - 1 {
                    ...
                    page_table.map_page_table(vaddr, paddr - KERNEL_OFFSET, level);
                    Ok(())
                }
            }
            _ => {
...
pub fn map_kernel_window(kernel_page_table: &mut PageTableImpl) -> ResultWithErr<String> {
    ...
    let mut start = 0;
    while start < end {
        start += wrapper.map_kernel(start + KERNEL_OFFSET, start + KERNEL_OFFSET)?;
    }
